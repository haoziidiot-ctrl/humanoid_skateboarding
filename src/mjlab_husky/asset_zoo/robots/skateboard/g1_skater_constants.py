...
G1_SIMPLE_XML: Path = Path(
  os.path.join(os.path.dirname(__file__), "xmls", "g1_simple.xml")
)
assert G1_SIMPLE_XML.exists()
...
def get_g1_simple_spec() -> mujoco.MjSpec:
  xml_root = ET.parse(G1_SIMPLE_XML).getroot()
  actuator_elem = xml_root.find("actuator")
  if actuator_elem is not None:
    xml_root.remove(actuator_elem)
  xml_string = ET.tostring(xml_root, encoding="unicode")
  spec = mujoco.MjSpec.from_string(xml_string, assets=get_assets("meshes"))
  return spec
...
G1_SIMPLE_ACTUATOR_5020 = BuiltinPositionActuatorCfg(
  target_names_expr=(
    ".*_elbow_joint",
    ".*_shoulder_pitch_joint",
    ".*_shoulder_roll_joint",
...
G1_SIMPLE_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    G1_SIMPLE_ACTUATOR_5020,
    G1_ACTUATOR_7520_14,
    G1_ACTUATOR_7520_22,
    G1_ACTUATOR_ANKLE,
  ),
...
def get_g1_simple_robot_cfg() -> EntityCfg:
  """Get a fresh G1 simple robot configuration instance."""
  return EntityCfg(
    init_state=PUSH_INIT_KEYFRAME,
    collisions=(FULL_COLLISION,),
    spec_fn=get_g1_simple_spec,
    articulation=G1_SIMPLE_ARTICULATION,
  )
...
G1_SIMPLE_ACTION_SCALE: dict[str, float] = {}
for a in G1_SIMPLE_ARTICULATION.actuators:
  assert isinstance(a, BuiltinPositionActuatorCfg)
  e = a.effort_limit
  s = a.stiffness
  names = a.target_names_expr
  assert e is not None
  for n in names:
    G1_SIMPLE_ACTION_SCALE[n] = 0.25 * e / s
...
if __name__ == "__main__":
  ...
  simple_robot = Entity(get_g1_simple_robot_cfg())
  viewer.launch(simple_robot.spec.compile())
